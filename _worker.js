    ...
    async fetch(request, env) {
        ...
        if (request.method === 'OPTIONS') {
            return new Response(null, {
                ...
                headers: {
                    ...
                    'Access-Control-Allow-Headers': 'authorization, apikey, content-type, x-client-info, prefer, range, x-upsert, x-requested-with, x-supabase-api-version, accept-profile, content-profile, x-supabase-auth-token, cache-control, pragma',
        ...
        if (url.pathname.startsWith('/supabase-api/')) {
            ...
            try {
                ...
                corsHeaders.set('Access-Control-Allow-Headers', 'authorization, apikey, content-type, x-client-info, prefer, range, x-upsert, x-requested-with, x-supabase-api-version, accept-profile, content-profile, x-supabase-auth-token, cache-control, pragma');
                corsHeaders.set('Access-Control-Expose-Headers', 'content-range, range, x-supabase-api-version');
