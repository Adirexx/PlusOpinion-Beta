        ...
        const WithdrawalModal = ({ isOpen, onClose, balance, onConfirm }) => {
            ...
            return (
                <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-[320px] bg-[#000000] border border-white/10 rounded-3xl p-6 shadow-2xl animate-pop">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-[10px] uppercase font-bold text-muted tracking-wider mb-1.5 block">Amount (₹100 - ₹3,000)</label>
                                ...
                                <div className="text-right mt-1">
                                    <span className="text-[10px] text-muted">Available: ₹{balance.toLocaleString()}</span>
                                </div>
                            </div>
        ...
        const EarningsView = ({ onBack, onScroll, isHeaderVisible, earnings: rawEarnings, profile }) => {
            ...
            return (
                <div className="flex flex-col h-full bg-[#000000] animate-slide-in relative">
                    ...
                    <ViewWrapper onScroll={onScroll}>
                        ...
                        <div className="glass-panel p-6 rounded-2xl relative overflow-hidden group">
                            ...
                            <h1 className="font-heading text-4xl font-bold text-white mb-6">₹{earnings.total.toLocaleString('en-IN')}</h1>

                            <div className="grid grid-cols-2 gap-4 border-t border-white/10 pt-4">
                                <div>
                                    <p className="text-[10px] text-muted uppercase">Last 30 Days</p>
                                    <p className="text-lg font-bold text-white">+₹{earnings.lastMonth}</p>
                                </div>
                                <div>
                                    ...
                                    <div key={log.id} className="glass-panel rounded-xl p-4 flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] text-muted font-medium">{log.date} • <span className={log.status === 'Paid' ? 'text-accent-green' : 'text-accent-gold'}>{log.status}</span></p>
                                            </div>
                                        </div>
                                        <span className="font-heading font-bold text-sm text-white">{log.amount < 0 ? '-' : '+'}₹{Math.abs(log.amount)}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
        ...
        const RQSView = ({ onBack, onScroll, isHeaderVisible, profile }) => {
            ...
            return (
                <div className="flex flex-col h-full bg-[#000000] animate-slide-in">
                        ...
                        <div className="flex flex-col items-center">
                            ...
                            <div className="w-full space-y-3">
                                ...
                                <div className="glass-panel p-4 rounded-xl flex items-center justify-between">
                                    ...
                                    <span className="text-sm font-heading font-bold text-white">{profile.rqs_consistency}/20</span>
                                </div>
                                <div className="glass-panel p-4 rounded-xl flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-heading font-bold text-white">{profile.rqs_verification}/40</span>
                                </div>
                                <div className="glass-panel p-4 rounded-xl flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-heading font-bold text-white">{profile.rqs_impact}/30</span>
                                </div>
                                <div className="glass-panel p-4 rounded-xl flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-heading font-bold text-white">{profile.rqs_legacy_score || 0}/10</span>
                                </div>
                            </div>
                        </div>
                    </ViewWrapper>
                            ...
                            <div className="glass-panel p-4 rounded-xl flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-muted">Must be ≥ {rqsTarget}</p>
                                    </div>
                                </div>
                                ...
                                return (
                                    <div key={item.id || i} className="glass-panel p-4 rounded-xl flex items-center justify-between group active:scale-[0.99] transition-transform animate-slide-in" style={{ animationDelay: `${i * 50}ms` }}>
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center overflow-hidden border border-white/10 shadow-sm shrink-0">
                                                ...
                                                <div className={`w-full h-full flex items-center justify-center font-bold text-sm bg-gradient-to-br from-white to-gray-100 text-[#000000] ${BRAND_LOGOS[brandName] ? 'hidden' : ''}`}>
                                                    {brandName ? brandName[0].toUpperCase() : 'B'}
                                                </div>
                                            </div>
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[9px] text-neon/80 font-bold tracking-wider bg-neon/5 px-2 py-1 rounded border border-neon/10 cursor-default flex items-center gap-2 uppercase">
        ...
        const PartnerApplicationModal = ({ isOpen, onClose, onSubmit }) => {
                                ...
                                >
                                    ...
                                    <p className="text-[11px] text-muted">
                                        {selectedFile ? (
                                            ...
                                            <>Upload Photo of ID Document<br />(Max 5MB • JPG, PNG, PDF)</>
                                        )}
                                    </p>
            ...
            return (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="relative w-full max-w-md bg-[#000000] border border-white/10 rounded-3xl p-6 shadow-2xl animate-pop overflow-hidden">
                        <div className="flex justify-between items-center mb-6">
                            <div>
                ...
                <div className="fixed inset-0 z-[60] flex items-end justify-center sm:px-4">
                    ...
                    <div className="relative w-full sm:max-w-md bg-[#121212] border-t sm:border border-white/10 rounded-t-3xl p-6 pt-4 shadow-2xl animate-slide-up overflow-hidden max-h-[85vh] flex flex-col">
                        ...
                        <div className="glass-panel rounded-2xl p-5 mb-6 border border-white/5 relative overflow-hidden group flex flex-col animate-fade-in shadow-2xl">
                            ...
                            <p className="text-sm text-gray-200 leading-relaxed mb-3 font-light line-clamp-3">{renderTextWithMentions(post.text)}</p>
                            ...
                            {(post.media || (post.images && post.images.length > 0)) && (
                                <div className="w-full rounded-xl mb-3 border border-white/5 relative bg-black/20 overflow-hidden">
                                    {(post.media_type === 'video' && (!post.images || post.images.length === 0)) ? (
                                        ...
                                        <div className="relative w-full">
                                            <img src={(post.images && post.images.length > 0) ? post.images[0] : post.media} className="w-full h-auto max-h-40 object-cover opacity-90" />
                                            {post.images && post.images.length > 1 && (
                                                <div className="absolute top-2 right-2 bg-black/60 rounded px-1.5 py-0.5 text-[9px] text-white font-bold backdrop-blur-sm pointer-events-none">
        ...
        const App = () => {
            ...
            const renderContent = () => {
                ...
                if (view === 'brands') return <BrandInteractionView onBack={() => setView('dashboard')} onScroll={handleScroll} isHeaderVisible={navVisible} posts={brandInteractions} onShare={handleShare} profile={profile} />;
                                            ...
                                            return (
                                                <div key={i} className="snap-start shrink-0 glass-panel rounded-xl p-3 min-w-[100px] active:bg-white/5 transition-colors">
                                                    <div className="w-8 h-8 rounded bg-white flex items-center justify-center mb-2 shadow-sm overflow-hidden p-1">
                                                        ...
                                                        <div className={`w-full h-full flex items-center justify-center font-bold text-xs bg-white text-[#000000] ${BRAND_LOGOS[bName] ? 'hidden' : ''}`}>
                                                            {bName[0].toUpperCase()}
                                                        </div>
                                                    </div>
                                                    <h4 className="text-[11px] font-bold text-white mb-1 truncate">{bName}</h4>
                                                </div>
                                            )
                                        })}
                                    </div>
            ...
            const NavItem = ({ icon, label, isActive, onClick, isMySpace, badge }) => (
                ...
                >
                    <div className={`relative ${isMySpace ? 'myspace-trigger' : ''} p-1.5 rounded-xl ${isActive ? 'bg-white/5' : ''}`} >
                        ...
                        {/* Badge Logic */}
                        {badge > 0 && (
                            <div className="absolute -top-1 -right-1 flex items-center justify-center min-w-[18px] h-[18px] bg-blue-500 rounded-full border-2 border-[#000000] z-50 animate-pulse-subtle">
        ...
        const AuthModal = ({ isOpen, onClose, isClosable = true }) => {
            ...
            return (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                    ...
                    <div className="w-full max-w-md bg-[#000000] p-8 rounded-3xl shadow-2xl relative border border-white/10" onClick={e => e.stopPropagation()}>
                        {isClosable && (
