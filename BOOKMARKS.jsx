        ...
        const CommentItem = ({ comment, onReply }) => {
            ...
            return (
                <div className={`flex gap-3 text-sm animate-fade-in ${comment.isPending ? 'opacity-50' : ''}`}>
                    <div className="flex-shrink-0">
                        <Avatar src={comment.avatar} className="w-6 h-6 rounded-full border border-white/10 shrink-0 object-cover cursor-pointer" fallbackSize={12}
                            onClick={() => { vibrate(5); window.location.href = `PUBLIC POV PROFILE.HTML?id=${comment.user_id}`; }} />
                    </div>
                    <div className="flex-1">
                        <div className="flex items-baseline justify-between">
                            <div className="flex items-baseline gap-2">
                                <span className="font-bold text-white text-xs cursor-pointer hover:text-neon transition-colors"
                                    onClick={() => { vibrate(5); window.location.href = `PUBLIC POV PROFILE.HTML?id=${comment.user_id}`; }}>
                                    {comment.user}
                                </span>
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={handleLike} className={`flex items-center gap-1 transition-colors ${isLiked ? 'text-white' : 'text-muted'}`}>
                                    <Icon icon="ThumbsUp" size={12} style={{ fill: isLiked ? 'white' : 'none', color: isLiked ? 'white' : 'inherit' }} />
            ...
            return (
                ...
                >
                                ...
                                {userProfile && userProfile.id === (post.user_id || post.profiles?.id) ? (
                                    ...
                                    <>
                                        ...
                                        <button onClick={() => handleMoreAction('bookmark')} className="flex items-center gap-3 p-3 hover:bg-white/5 rounded-lg text-left text-xs text-white">
                                            <Icon icon={isSaved ? "BookmarkFilled" : "Bookmark"} size={16} className={isSaved ? "text-neon fill-neon" : "text-muted"} />
                    ...
                    <div className="flex items-center flex-wrap gap-2 mb-3 mt-1">
                        ...
                        {post.verified && (
                            <div className="flex items-center text-accent-green touch-scale group relative" title="Verified Purchase">
        ...
        const SmartMedia = ({ src, type, images, onImageClick }) => {
            ...
            const onCarouselTouchEnd = () => {
                ...
                setIsDragging(false);
                if (dragDelta < -SWIPE_THRESHOLD && activeIdx < imageList.length - 1) setActiveIdx(i => i + 1);
                else if (dragDelta > SWIPE_THRESHOLD && activeIdx > 0) setActiveIdx(i => i - 1);
            ...
            return (
                <div ref={containerRef} className={`w-full rounded-xl mb-3 border border-white/5 relative bg-black overflow-hidden cursor-pointer select-none media-zoom-effect ${aspectClass}`} style={{ zIndex: isPinching ? 50 : 1 }} onClick={handleClick}>
                    {isVideo ? (
                        <div className="absolute inset-0" onTouchStart={onSingleTouchStart} onTouchMove={onSingleTouchMove} onTouchEnd={onSingleTouchEnd} style={{ transform: `scale(${scale})`, transformOrigin: `${pinchOrigin.x}% ${pinchOrigin.y}%`, transition: isPinching ? 'none' : 'transform 0.3s' }}>
                            <video src={src} className="w-full h-full object-cover pointer-events-none" playsInline loop muted={isMuted || !isInView} autoPlay onLoadedMetadata={handleMediaLoad} />
                            <button onClick={toggleMute} className="absolute bottom-3 right-3 bg-black/50 hover:bg-black/70 backdrop-blur-sm rounded-full p-2 text-white transition-all z-10 pointer-events-auto">
                                {isMuted ? (
                        ...
                        <>
                            ...
                            >
                                {imageList.map((imgSrc, idx) => (<div key={idx} className="h-full flex-shrink-0" style={{ width: `${100 / imageList.length}%` }}><img src={imgSrc} alt={`Image ${idx + 1}`} loading={idx === 0 ? 'eager' : 'lazy'} className="w-full h-full object-cover pointer-events-none" onLoad={idx === 0 ? handleMediaLoad : undefined} draggable="false" /></div>))}
                            </div>
                            <div className="absolute bottom-2.5 left-0 right-0 flex justify-center gap-1.5 pointer-events-none z-10">{imageList.map((_, idx) => (<div key={idx} className="transition-all duration-300" style={{ width: idx === activeIdx ? '18px' : '6px', height: '6px', borderRadius: '3px', background: idx === activeIdx ? 'white' : 'rgba(255,255,255,0.4)' }} />))}</div>
                            <div className="absolute top-2.5 right-2.5 bg-black/50 backdrop-blur-sm rounded-full px-2 py-0.5 text-[10px] text-white font-bold pointer-events-none z-10">{activeIdx + 1}/{imageList.length}</div>
                        </>
                    ) : (
                        <div className="absolute inset-0" onTouchStart={onSingleTouchStart} onTouchMove={onSingleTouchMove} onTouchEnd={onSingleTouchEnd} style={{ transform: `scale(${scale})`, transformOrigin: `${pinchOrigin.x}% ${pinchOrigin.y}%`, transition: isPinching ? 'none' : 'transform 0.3s' }}>
                            <img src={src} alt="Post media" loading="lazy" className="w-full h-full object-cover pointer-events-none" onLoad={handleMediaLoad} draggable="false" />
                        </div>
                    )}
                </div>
            );
        ...
        const ImageViewer = ({ src, type, images, initialIndex = 0, onClose }) => {
            ...
            const handleTouchStart = (e) => {
                if (e.touches.length === 2) { setIsPinching(true); const t1 = e.touches[0], t2 = e.touches[1]; initialDist.current = Math.hypot(t1.pageX - t2.pageX, t1.pageY - t2.pageY); lastScale.current = scale; }
                else if (e.touches.length === 1) { if (scale > 1) { lastTouch.current = { x: e.touches[0].pageX - position.x, y: e.touches[0].pageY - position.y }; } else if (isMulti) { dragStart.current = e.touches[0].clientX; setIsDragging(true); } }
            };
            const handleTouchMove = (e) => {
                if (e.touches.length === 2 && isPinching && initialDist.current) { e.preventDefault(); const t1 = e.touches[0], t2 = e.touches[1]; setScale(Math.min(Math.max((Math.hypot(t1.pageX - t2.pageX, t1.pageY - t2.pageY) / initialDist.current) * lastScale.current, 1), 5)); }
                else if (e.touches.length === 1 && scale > 1) { const lx = (scale - 1) * (window.innerWidth / 2), ly = (scale - 1) * (window.innerHeight / 2); setPosition({ x: Math.min(Math.max(e.touches[0].pageX - lastTouch.current.x, -lx), lx), y: Math.min(Math.max(e.touches[0].pageY - lastTouch.current.y, -ly), ly) }); }
                else if (e.touches.length === 1 && isDragging && isMulti && scale <= 1) { const delta = e.touches[0].clientX - dragStart.current; setDragDelta((currentIdx === 0 && delta > 0) || (currentIdx === imgList.length - 1 && delta < 0) ? delta * 0.2 : delta); }
            };
            ...
            return (
                <div className="fixed inset-0 z-[100] bg-black animate-fade-in overflow-hidden" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
                    <button className="absolute top-5 right-5 p-2.5 bg-white/10 rounded-full text-white z-50 backdrop-blur-sm" onClick={onClose}><Icon icon="X" size={22} /></button>
                    {isMulti && <div className="absolute top-5 left-1/2 -translate-x-1/2 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1 text-sm text-white font-bold z-50">{currentIdx + 1} / {imgList.length}</div>}
                    {isVideo ? (
                        <div className="w-full h-full flex items-center justify-center p-4" onClick={onClose}><video src={src} className="max-w-full max-h-full object-contain" controls autoPlay onClick={e => e.stopPropagation()} /></div>
                    ) : isMulti ? (
                        <>
                            <div className="absolute inset-0 flex items-center" style={{ transform: `translateX(calc(${-currentIdx * (100 / imgList.length)}% + ${dragDelta}px))`, transition: isDragging ? 'none' : 'transform 0.32s cubic-bezier(0.25,0.46,0.45,0.94)', width: `${imgList.length * 100}%` }}>
                                {imgList.map((imgSrc, idx) => (<div key={idx} className="h-full flex items-center justify-center flex-shrink-0" style={{ width: `${100 / imgList.length}%` }}><img src={imgSrc} alt={`Image ${idx + 1}`} className="max-w-full max-h-full object-contain select-none p-2" style={idx === currentIdx ? { transform: `translate(${position.x}px, ${position.y}px) scale(${scale})`, transition: isPinching ? 'none' : 'transform 0.1s' } : {}} onDoubleClick={idx === currentIdx ? handleDoubleTap : undefined} draggable="false" onClick={e => e.stopPropagation()} /></div>))}
                            </div>
                            <div className="absolute bottom-8 left-0 right-0 flex justify-center gap-2 pointer-events-none z-50">{imgList.map((_, idx) => (<div key={idx} style={{ width: idx === currentIdx ? '20px' : '6px', height: '6px', borderRadius: '3px', background: idx === currentIdx ? 'white' : 'rgba(255,255,255,0.4)', transition: 'all 0.3s' }} />))}</div>
                        </>
                    ) : (
                        <div className="w-full h-full flex items-center justify-center" onClick={onClose}><img src={imgList[0] || src} className="max-w-full max-h-full object-contain p-2 select-none" style={{ transform: `translate(${position.x}px, ${position.y}px) scale(${scale})`, transition: isPinching ? 'none' : 'transform 0.1s ease-out' }} onDoubleClick={handleDoubleTap} onClick={e => e.stopPropagation()} draggable="false" /></div>
                    )}
                </div>
            );
        };
                ...
                <>
                    ...
                    >
                        ...
                        <div className="relative flex-1 flex flex-col overflow-y-auto no-scrollbar px-6 pt-safe pb-safe">
                            ...
                            <div className="relative rounded-2xl p-5 mb-8 overflow-hidden group shrink-0 active:scale-[0.99] transition-transform duration-200 cursor-pointer" onClick={() => window.location.href = 'PRIVATE OWNER PROFILE.HTML'}>
                                ...
                                <div className="relative z-10 flex flex-col items-start">
                                    <div className="flex items-start justify-between w-full mb-4">
                                        ...
                                        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-cyan-900/60 to-blue-900/60 border border-neon/30 rounded-full shadow-[0_0_10px_rgba(47,139,255,0.15)]">
                                            ...
                                            <span className="text-cyan-100 text-[11px] font-heading font-bold tracking-wider uppercase">RQS {userProfile.rqs}</span>
                                        </div>
                                    </div>
                            ...
                            <div className="flex-1 space-y-8">
                                <div>
                                    <div className="text-[11px] font-bold text-muted uppercase tracking-widest mb-3 px-2">Settings</div>
                                    <div className="space-y-3">
                                        <MenuItem icon="Bookmark" label="Bookmarks" onClick={() => { window.location.href = 'BOOKMARKS.HTML' }} />
                                ...
                                <div>
                                    <div className="text-[11px] font-bold text-muted uppercase tracking-widest mb-3 px-2">Support</div>
                                    <div className="space-y-3">
                                        <MenuItem icon="LifeBuoy" label="Support & Legal" onClick={() => { }} />
                ...
                >
                    ...
                    <div className="px-4 py-3 flex gap-2 overflow-x-auto no-scrollbar border-b border-white/5 bg-[#020205] ptr-ignore">
                        {LENS_TOPICS.map(topic => (
                            ...
                            >
                                {topic.icon && <Icon icon={topic.icon} size={12} />}
                                {topic.label}
                            </button>
                        ))}
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar bg-[#020205] pb-24 ptr-enabled">
                        ...
                        {/* Static Sections only when NOT searching (optional, or keep them) */}
                        {!searchQuery && (
                            <>
                                <div className="px-4 py-6">
                                    <h3 className="text-[10px] font-bold text-muted uppercase tracking-widest mb-3 pl-1">Quick Searches</h3>
                                    <div className="flex flex-wrap gap-2">
                                        {LENS_SEARCHES.map(tag => (
                                            <button key={tag} className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-white/80 flex items-center gap-1.5 hover:border-neon/50 hover:text-white transition-all active:scale-95" onClick={() => setSearchQuery(tag)}>
                                                <Icon icon="Search" size={10} className="text-neon" /> {tag}
                                            </button>
                                        ))}
                                    </div>
                                </div>
            ...
            return (
                <div className="fixed inset-0 z-[60] flex items-end justify-center sm:px-4">
                    ...
                    <div className="relative w-full sm:max-w-md bg-[#121212] border-t sm:border border-white/10 rounded-t-3xl p-6 pt-4 shadow-2xl animate-slide-up overflow-hidden max-h-[85vh] flex flex-col">
                        ...
                        <div className="glass-panel rounded-2xl p-5 mb-6 border border-white/5 relative overflow-hidden group flex flex-col animate-fade-in">
                            ...
                            {post.media && (
                                <div className="w-full rounded-xl mb-3 border border-white/5 relative bg-black/20 overflow-hidden">
                                    {post.media_type === 'video' ? (
                                        ...
                                        <img src={post.media} className="w-full h-auto max-h-40 object-cover opacity-90" />
                                    )}
                                </div>
                            )}
        ...
        const ReportModal = ({ isOpen, onClose, onSubmit }) => {
            ...
            return (
                <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
                    ...
                    <div className="relative w-full max-w-sm bg-[#121212] border border-white/10 rounded-2xl overflow-hidden shadow-2xl animate-scale-up">
                        ...
                        <div className="p-4 border-b border-white/5 flex items-center justify-between bg-[#1A1A1A]">
                            <span className="font-heading font-bold text-white flex items-center gap-2">
                                <Icon icon="AlertTriangle" size={18} className="text-red-500" />
                                Report Opinion
                            </span>
                            <button onClick={onClose} className="text-white/40 hover:text-white"><Icon icon="X" size={20} /></button>
                        </div>
                    ...
                    <div className="flex-1 min-w-0 pt-0.5">
                        <div className="flex justify-between items-start mb-1">
                            ...
                            <div className="flex items-center justify-center">
                                <Icon icon="BookmarkFilled" size={18} className="text-neon fill-neon" />
                            </div>
                        </div>
                        <p className="text-[13px] text-slate-400 leading-relaxed line-clamp-2">{message}</p>
                        <div className="mt-3 flex items-center justify-between">
                            {tag ? (
                                <span className="text-[10px] font-bold px-2.5 py-1 rounded bg-white/5 text-neon border border-neon/20">{tag}</span>
                            ) : <div></div>}
                            <button
        ...
        const App = () => {
            ...
            const handleRemovePost = async (postId, reason = 'Removed', type = 'delete', target = null) => {
                ...
                try {
                    ...
                    else if (type === 'mute_brand') await window.hideItem('brand', target || 'Brand');
                    else if (type === 'not_interested') await window.hideItem('category', target || 'General');
                    else if (type === 'Deleted') await window.deletePost(postId);
            ...
            return (
                ...
                >
                    ...
                    {/* TOP NAV - BOOKMARKS HEADER */}
                    <div className={`top-nav-glass fixed top-0 left-0 w-full h-[60px] flex items-center px-5 z-30 transition-transform duration-500 ease-out ${navVisible ? 'translate-y-0' : '-translate-y-full'}`}>
                        <button className="p-3 -ml-2 text-white/80 hover:text-white transition-colors relative z-10 touch-scale" onClick={() => { if (history.length > 1) { history.back(); } else { window.location.href = 'index.html'; } }}>
                            <Icon icon="ChevronLeft" size={24} />
                        </button>
                        <div className="flex-1 flex justify-center items-center -ml-6">
                            <span className="font-heading font-bold text-lg tracking-wide text-white">Bookmarks</span>
                        </div>
                    </div>
                    ...
                    >
                        {loading ? (
                            ...
                            <div className="animate-fade-in">
                                <div className="sticky top-0 z-[40] bg-[#020205] border-b border-white/[0.05] flex p-1 m-4 rounded-xl bg-white/[0.03]">
                                    <button
                                    ...
                                    >
                                        Saved Alerts ({bookmarkedNotifs.length})
                                    </button>
                                </div>

                                {bookmarkTab === 'opinions' ? (
                                    <div className="space-y-4 px-4 pb-32">
                                        ...
                                        {!isLoadingPosts && hasMore && (
                                            <div className="flex justify-center py-8">
                                                <button onClick={() => setOffset(prev => prev + 20)} className="text-xs font-bold text-neon tracking-widest hover:opacity-80 transition-opacity">LOAD MORE</button>
                                            </div>
                                        )}

                                        {!isLoadingPosts && visiblePosts.length > 0 && !hasMore && (
                                            <div className="py-20 flex flex-col items-center justify-center gap-4 opacity-40">
                                                <div className="w-16 h-[1px] bg-gradient-to-r from-transparent via-neon to-transparent"></div>
                                                <span className="text-[10px] tracking-[0.3em] font-heading text-neon">END OF FEED</span>
                                                <div className="w-16 h-[1px] bg-gradient-to-r from-transparent via-neon to-transparent"></div>
                                            </div>
                                        )}
                                        ...
                                        {!isLoadingPosts && visiblePosts.length === 0 && (
                                            <div className="flex flex-col items-center justify-center py-20 text-center opacity-60">
                                                <Icon icon="Bookmark" size={48} className="text-muted mb-4 stroke-1" />
                                                <p className="text-white font-heading font-bold text-lg">No Bookmarks Yet</p>
                                                <p className="text-muted text-sm max-w-xs mt-2">Save opinions you want to read later.</p>
                                                <button onClick={() => window.location.href = 'HOMEPAGE_FINAL.HTML'} className="mt-6 px-6 py-2 bg-white/10 rounded-full text-white text-xs font-bold border border-white/5 hover:bg-white/20 transition-colors">Go to Feed</button>
                                            </div>
                                        )}
                                    </div>
                                ) : (
                                    <div className="pb-32 min-h-[400px]">
                                        {isLoadingNotifs ? (
                                            <div className="flex justify-center items-center py-20">
                                                <div className="w-6 h-6 border-2 border-neon border-t-transparent rounded-full animate-spin"></div>
                                            </div>
                                        ) : bookmarkedNotifs.length > 0 ? (
                                            <div className="space-y-[1px]">
                                                {bookmarkedNotifs.map(notif => (
                                                    <NotificationBookmarkItem
                                                        key={`notif-${notif.id}`}
                                                        data={notif}
                                            ...
                                            <div className="flex flex-col items-center justify-center py-24 text-center opacity-60 px-6">
                                                <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-6">
                                                    <Icon icon="Bell" size={32} className="text-muted stroke-1" />
                                                </div>
                                                <p className="text-white font-heading font-bold text-lg">No Saved Alerts</p>
                                                <p className="text-muted text-sm max-w-[200px] mt-2 leading-relaxed font-light">
                                                    Swipe right on a notification to save it for later reference.
                                                </p>
                                            </div>
                                        )}
                                    </div>
                    ...
                    {/* ONBOARDING MODAL POPUP for logged in users with incomplete profile */}
                    {showOnboardingModal && !isGuest && (
                        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                            ...
                            <div className="relative flex flex-col items-center gap-6 text-center">
                                ...
                                <div>
                                    <h2 className="text-white font-heading font-bold text-xl mb-2">Complete Your Profile</h2>
                                    <p className="text-muted text-sm max-w-xs">You need to set up your profile before using PlusOpinion.</p>
                                </div>
                                <button
